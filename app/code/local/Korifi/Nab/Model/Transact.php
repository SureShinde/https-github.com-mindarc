<?php
class Korifi_Nab_Model_Transact extends Mage_Payment_Model_Method_Cc
{
	...
	public function capture(Varien_Object $payment, $amount)
	{
		...
		else
		{
			...
			if ($result['Status']['statusCode'] == "000")
			{
				...
				{
					Mage::throwException("Bank error " . $result['Payment']['TxnList']['Txn']['responseCode'] . ": " . $result['Payment']['TxnList']['Txn']['responseText']);
				}

			}
			else
			{
				Mage::throwException("Gateway error " . $result['Status']['statusCode'] . ": " . $result['Status']['statusDescription']);
			}
		}
		return $this;
	}
