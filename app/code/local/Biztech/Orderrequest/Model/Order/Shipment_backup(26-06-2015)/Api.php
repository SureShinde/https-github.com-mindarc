 ...
 */
class Biztech_Orderrequest_Model_Order_Shipment_Api extends Mage_Sales_Model_Api_Resource
{
    ...
    public function info($shipmentIncrementId)
    {
        ...
        /** Added By BD: NT : 24-06-2015
        *  Task: add order comments in shipping address field array
        *  starts
        */
        ...
        foreach ($_history as $_historyItem){
            if (!$_historyItem->isDeleted() && $_historyItem->getComment() && $_historyItem->getIsVisibleOnFront()) {
                $result['customer_comments'][$comment_index]['comment'] = strip_tags(str_replace('Customer Order Comment','',$_historyItem->getComment()));
                $result['customer_comments'][$comment_index]['create_at'] = $_historyItem->getCreatedAt();
                $comment_index++;
            }
        }
        /* ends */
