<?php
    class Biztech_Auspost_Model_Carrier_Auspost extends Mage_Shipping_Model_Carrier_Abstract implements Mage_Shipping_Model_Carrier_Interface {
        ...
        public function collectRates(Mage_Shipping_Model_Rate_Request $request)
        {
			...
			if(!$_servicesArr){
                $count = Mage::getModel('core/session')->getAusExceptionCount();
                if(isset($count)){
                    $ausExceptionCount = intval(Mage::getModel('core/session')->getAusExceptionCount())+1;
                }
                if($ausExceptionCount <= 2){
                    Mage::getModel('core/session')->setAusExceptionCount($ausExceptionCount);
                    Mage::log('ReCall function after Exception : '.$ausExceptionCount,null,'ausExceptionCount.log');
                    $this->collectRates($request);
                }else{
                    Mage::getModel('core/session')->unsAusExceptionCount();
                }
